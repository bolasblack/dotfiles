...
let
  ...
  caskToDerivation = { cask, config }: stdenv.mkDerivation rec {
    pname = cask.token;
    ...
    src = pkgs.fetchurl {
      url = cask.url;
      hash =
        if !isNull config.srcHash then
          config.srcHash
        else
          lib.optionalString (cask.sha256 != "no_check") (builtins.convertHash {
            hash = cask.sha256;
            toHashFormat = "sri";
            hashAlgo = "sha256";
          });
...
builtins.listToAttrs
  (builtins.map
    ...
    let
      _ = lib.assertMsg
        (lib.strings.versionAtLeast builtins.nixVersion "2.19")
        "nix version must be 2.19 or higher";
      defaultConfig = {
        srcHash = null;
      };
      originDrv = caskToDerivation {
        inherit cask;
      ...
      value =
        originDrv // {
          withConfig = getConfig:
            caskToDerivation {
              inherit cask;
              config = defaultConfig // (getConfig { inherit cask; });
            };
        };
    })
    casks)
